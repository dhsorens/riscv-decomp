...
- **Two `inr` branches need not reach the same machine label.**
  ...
  needs no join; use it when the exits genuinely diverge. A two-exit total
  judgement with existential arms was built for the caller holding only
  `TerminatesB` and withdrawn — take the derivation instead.
